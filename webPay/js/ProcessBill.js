...
if (event.origin !== window.location.origin)
    params = event.data
    origin = event.origin
    $("#detail_amount").html(`${params.amount} ${params.currency}`)
...
function bill({elem, amount, successUrl, currency, description, businessName, orderId, apiKey}) {
...
//const amount = atob(amount) ///en court
...
case '#momo':
    ...
    $.ajax({
        ...
        data: JSON.stringify({ amount, wallet, currency, description, payer, orderId})
    })
    .done(function(res){
            callback(res) 
    })
    ...
    case '#eu':
        ...
        $.ajax({
            ...
            data: JSON.stringify({ amount, currency, description, payer, orderId})
        })
        .done(function (res){
            if(res.success){
                $.alert({
        ...
        case '#paypal':
            ...
            $.ajax({
                ...
                data: JSON.stringify({ amount, wallet, currency, payer, orderId})
            })
            .done(function (res){
                if(res.success){
            ...
            case '#pm':
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({ amount, currency, payer, orderId})
                })
                .done(function (res){
                    if(res.success){
